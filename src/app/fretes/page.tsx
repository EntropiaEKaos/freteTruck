  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <p className="mt-1 text-slate-600 dark:text-slate-400">
        {rows.length} {rows.length === 1 ? "frete encontrado" : "fretes encontrados"}
      </p>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
        ...
        <form method="GET" className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 space-y-4 lg:sticky lg:top-20">
          ...
          <div>
            <label className={labelCls}>Busca livre</label>
            <input name="q" defaultValue={q} placeholder="Carga, cidade..." className={inputCls} />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Origem UF</label>
              <select name="originState" defaultValue={originState} className={inputCls}>
                <option value="">Todas</option>
                {UFS.map((uf) => <option key={uf} value={uf}>{uf}</option>)}
              </select>
            </div>
            <div>
              <label className={labelCls}>Destino UF</label>
              <select name="destState" defaultValue={destState} className={inputCls}>
                <option value="">Todas</option>
                {UFS.map((uf) => <option key={uf} value={uf}>{uf}</option>)}
              </select>
            </div>
          ...
          <div>
            <label className={labelCls}>Cidade origem</label>
            <input name="originCity" defaultValue={originCity} placeholder="Ex: Sorriso" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Cidade destino</label>
            <input name="destCity" defaultValue={destCity} placeholder="Ex: Santos" className={inputCls} />
          </div>

          <div>
            <label className={labelCls}>Tipo de caminhão</label>
            <select name="vehicle" defaultValue={vehicle} className={inputCls}>
          ...
          <div>
            ...
            <select name="body" defaultValue={body} className={inputCls}>
              <option value="">Todas</option>
          ...
          <div className="flex gap-2 pt-1">
            <button type="submit" className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-bold py-2.5 rounded-lg text-sm transition-colors">
              Filtrar
            </button>
            <a href="/fretes" className="px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
              Limpar
            </a>
          </div>
        </form>
        ...
        <div>
          {rows.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-12 text-center">
              ...
              <p className="mt-1 text-slate-500 dark:text-slate-400 text-sm">Tente ajustar os filtros ou volte mais tarde — novos fretes são publicados o tempo todo.</p>
            </div>
          ) : (
            <>
