...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const description = `${f.cargoType} · ${formatWeight(f.weightKg)} · ${f.distanceKm ? f.distanceKm + " km" : ""} · Veículos: ${f.vehicleTypes}. Negocie direto pelo FreteTruck.`;
  return {
    title: `${title} | FreteTruck`,
    description,
...
export default async function FreightDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { freight: f, ownerName, ownerCompany } = rows[0];

  // Fretes de retorno
  const returnFreights = await db
    .select({ freight: freights, ownerName: users.name, ownerCompany: users.company })
    .from(freights)
    .innerJoin(users, eq(freights.userId, users.id))
    .where(and(eq(freights.status, "ativo"), eq(freights.originState, f.destState), ne(freights.id, f.id)))
    .orderBy(desc(freights.createdAt))
    .limit(3);

  // Fretes similares (mesmo tipo de carga)
  const similarFreights = await db
    .select({ freight: freights, ownerName: users.name, ownerCompany: users.company })
    .from(freights)
    .innerJoin(users, eq(freights.userId, users.id))
    .where(and(eq(freights.status, "ativo"), eq(freights.cargoType, f.cargoType), ne(freights.id, f.id)))
    .orderBy(desc(freights.createdAt))
    .limit(3);
  ...
  const priceLabel =
    f.priceType === "combinar" || !f.price
      ...
      : f.priceType === "tonelada"
        ? `${formatBRL(f.price)} por tonelada`
        : formatBRL(f.price);

  const perKm = f.price && f.distanceKm && f.priceType === "total"
    ? (parseFloat(f.price) / f.distanceKm)
    : null;

  const phone = f.contactPhone.startsWith("55") ? f.contactPhone : `55${f.contactPhone}`;
  const waText = encodeURIComponent(
    `Olá! Vi seu frete no FreteTruck: ${f.cargoType} de ${f.originCity}/${f.originState} para ${f.destCity}/${f.destState}. Ainda está disponível?`
  );
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {(() => {
        ...
        return (
          <div className="flex items-center justify-between flex-wrap gap-2">
            <Link href="/fretes" className="text-sm text-orange-600 font-semibold hover:underline">
              ← Voltar para a busca
            </Link>
            <div className="flex items-center gap-2 flex-wrap">
              <FreightQRCode freightId={f.id} title={routeTitle} price={priceLabel} />
              <Link
                href={`/comprovante/${f.id}`}
                ...
                title="Ver Comprovante de Entrega Digital (POD)"
              >
                📄 Comprovante POD
              </Link>
      ...
      <div className="mt-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="bg-slate-900 dark:bg-black text-white p-6 md:p-8">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="text-xs uppercase tracking-widest text-orange-400 font-semibold">
                Frete #{f.id} · {timeAgo(f.createdAt)} · {f.views + 1} visualizações
              </p>
            ...
            <div className="text-right">
              ...
              {perKm && (
                <p className="text-sm text-slate-300 mt-0.5">
                  R$ {perKm.toFixed(2)}/km {perKm >= 7 ? "🟢" : perKm >= 5 ? "🟡" : "🔴"}
                </p>
              )}
          ...
          {f.status === "fechado" && (
            <div className="mt-4 bg-red-500/20 border border-red-500 text-red-200 rounded-lg px-4 py-2 text-sm font-semibold">
              ⚠️ Este frete já foi fechado.
            </div>
          )}
        </div>
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            {f.description && (
              <>
                <h2 className="font-bold text-slate-900 dark:text-white mt-6 mb-2">Observações</h2>
                <p className="text-sm text-slate-600 dark:text-slate-300 whitespace-pre-line">{f.description}</p>
              </>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {f.vehicleTypes.split(",").map((v) => (
                <span key={v} className="text-sm bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-700 px-3 py-1 rounded-full font-medium">
                  🚚 {v}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {f.bodyTypes.split(",").map((b) => (
                <span key={b} className="text-sm bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600 px-3 py-1 rounded-full font-medium">
                  {b}
                </span>
              ))}
            </div>
            ...
            <div className="mt-6 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
              ...
              <Link href={`/perfil/${f.userId}`} className="mt-1 font-bold text-slate-900 dark:text-white hover:text-orange-600 dark:hover:text-orange-400 transition-colors">
              ...
              {f.status === "ativo" ? (
                <div className="mt-4 space-y-2">
                  ...
                  <a
                    href={`/chat?with=${f.userId}`}
                    className="flex items-center justify-center gap-2 w-full bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white font-bold py-3 rounded-xl transition-colors"
                  >
                    ✉️ Chat interno
                  </a>
                </div>
              ) : (
                <div className="mt-4 text-center text-sm text-slate-400 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl py-3">
                  Contato indisponível — frete fechado
                </div>
              )}
              <p className="mt-2 text-center text-xs text-slate-400">
                <Link href={`/perfil/${f.userId}`} className="text-orange-600 hover:underline">Ver perfil e avaliações →</Link>
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Proposta online, favorito e avaliações */}
      <div className="mt-6">
        <FreightActions freightId={f.id} ownerId={f.userId} ownerName={ownerName} freightStatus={f.status} />
      </div>
      ...
      {similarFreights.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white">
            📦 Fretes similares ({f.cargoType})
          </h2>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            {similarFreights.map((row) => (
